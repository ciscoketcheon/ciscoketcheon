...
framework.hears('test', function (bot, trigger) {
	  console.log("test command received");
	    responded = true;
	      let outputString = `This is a test output message`;
	  console.log(outputString);
	  bot.say("markdown", outputString);
});

//Run python script time
framework.hears('esa-time', function (bot) {
  console.log("Run Python script command received - esa-time");
  responded = true;

  // Use child_process.exec to run the Python script
  exec('python3 esa-time.py', (error, stdout, stderr) => {
    if (error) {
      console.error(`Error executing Python script: ${error.message}`);
      bot.say("markdown", "There was an error while running the Python script.");
      return;
    }
    ...
    // Send the script's output as a message to the Webex space
    bot.say("markdown", `Python script output:\n\n${stdout}`);
  });
});


//Run python script traffic
framework.hears('esa-in', function (bot) {
  console.log("Run Python script command received - esa-in");
  responded = true;

      // Use child_process.exec to run the Python script
  exec('python3 esa-in.py', (error, stdout, stderr) => {
   if (error) {
      console.error(`Error executing Python script: ${error.message}`);
      bot.say("markdown", "There was an error while running the Python script.");
      return;
...
function sendHelp(bot) {
  bot.say("markdown", 'These are the commands I can respond to:', '\n\n ' +
    ...
    'Script test [BETA] \n' +
    '**esa-time** Get ESA current time \n' +
    '**esa-in** Incoming policy hit \n');
}
